...
class CUEventoController(AbstractCUController):
    ...
    def _inizia_salvataggio(self, is_new: bool) -> None:
        ...
        if is_new:
            ...
            data = cur_pagina.data.date()
            time = cur_pagina.ora.time()

            py_date = QDateTime(data, time).toPyDateTime()

            # Tenta di creare il nuovo evento
            try:
                nuovo_evento = Evento(py_date, cur_pagina.id_spettacolo)
            except DatoIncongruenteException as exc:
                # È stato trovato un campo con input non valido
                cur_pagina.show_input_error(DATI_INCONGRUENTI)
            ...
            copia_evento = self.__get_evento(cur_pagina.cur_id_evento)
            if not isinstance(copia_evento, Evento):
                # Non esiste evento con l'id salvata nella pagina
                PopupMessage.mostra_errore(
                    cur_pagina,
                    "Errore nel salvataggio",
                    f"Non è presente nessun evento con id {cur_pagina.cur_id_evento}. "
